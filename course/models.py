...
class Activity(models.Model):
    ...
    audio_file = models.FileField(
        ...
        blank=True,
        null=True,
        help_text="Upload an MP3 file",
    )
    pdf_file = models.FileField(
        upload_to=pdf_media_path, blank=True, null=True, help_text="Upload a PDF file"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Item(models.Model):
    ...
    audio_play = models.CharField(
        max_length=6, choices=PLAY_AUDIO, default="start", null=True, blank=True
    )
    number_answers = models.SmallIntegerField(
        default=1, blank=True, null=True
    )  # Number of answers for MCQ
